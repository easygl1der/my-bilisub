...
使用示例:
    python ai_xiaohongshu_homepage_v2.py
"""
        ...
        for i in range(refresh_count):
            ...
            try:
                ...
                for link_info in links:
                    # 从链接信息中提取数据
                    url = link_info['url']
                    note_id = link_info['noteId']
                    xsec_token = link_info.get('xsecToken', '')
                    xsec_source = link_info.get('xsecSource', 'pc_homepage')
                    ...
                    # 获取标题
                    try:
                        title_elem = await page.query_selector(f'a[href="{url}"] span')
                        title = await title_elem.text_content() if title_elem else "无标题"
                        title = title.strip()[:50]
                    except:
                        title = "无标题"

                    # 构建完整链接（包含 xsec_token）
                    if xsec_token:
                    ...
                    note_data = {
                        ...
                        '笔记ID': note_id,
                        '类型': 'unknown',
                        'xsec_token': xsec_token,  # 新增字段
                        'xsec_source': xsec_source,  # 新增字段
                        '采集时间': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    }

                    notes_collected.append(note_data)
                    print(f"    ✓ [{len(notes_collected)}] {title}")
...
def export_to_csv(notes, output_path):
    """导出笔记到CSV"""
    csv_columns = ['序号', '标题', '链接', '完整链接', '笔记ID', '类型', 'xsec_token', 'xsec_source', '采集时间']
