...
使用方法:
    python fetch_xhs_image_notes.py
...
PROJECT_ROOT = SCRIPT_DIR.parent.parent  # 获取项目根目录 (bilisub)
...
OUTPUT_DIR = PROJECT_ROOT / "output" / "xhs_images"
...
def filter_image_notes(notes: List[Dict]) -> List[Dict]:
    """过滤出图文笔记（排除视频）"""
    image_notes = []
    for note in notes:
        # 根据笔记类型过滤
        ...
        if note_type == 'normal' or (note.get('image_count', 0) > 0 and not note.get('video_url')):
            image_notes.append(note)

    print(f"✅ 筛选出图文笔记: {len(image_notes)}/{len(notes)}")
    return image_notes
...
def save_to_csv(notes: List[Dict], user_id: str, output_dir: Path) -> Optional[Path]:
    ...
    csv_file = output_dir / f"xhs_images_{user_id}_{timestamp}.csv"
        ...
        with open(csv_file, 'w', encoding='utf-8-sig', newline='') as f:
            ...
            for i, note in enumerate(notes, 1):
                row = {
                    ...
                    '类型': note.get('type', 'normal'),
                    '图片数量': note.get('image_count', 0),
...
def save_to_markdown(notes: List[Dict], user_id: str, output_dir: Path) -> Optional[Path]:
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    md_file = output_dir / f"xhs_images_{user_id}_{timestamp}.md"

    try:
        with open(md_file, 'w', encoding='utf-8') as f:
            f.write(f"# 小红书用户图文笔记汇总\n\n")
            f.write(f"**用户ID**: {user_id}\n\n")
            f.write(f"**获取时间**: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            f.write(f"**图文数量**: {len(notes)}\n\n")
            f.write("---\n\n")
            f.write(f"## 图文列表\n\n")
            f.write(f"| 序号 | 标题 | 链接 | 笔记ID | 图片数 | 发布时间 | 点赞 | 收藏 | 评论 |\n")
            f.write(f"|------|------|------|--------|--------|----------|------|------|------|\n")
            ...
            for i, note in enumerate(notes, 1):
                ...
                f.write(f"| {i} | {title} | [链接]({url}) | {note_id} | {image_count} | {publish_time} | {like_count} | {collect_count} | {comment_count} |\n")
...
async def main_async(args):
    ...
    if not notes:
        print("\n❌ 未获取到笔记")
        print("\n💡 建议:")
        print("   1. 手动提供图文链接列表CSV文件")
        print("   2. 或者配置MediaCrawler以使用完整API")
        return False

    # 步骤4: 过滤图文笔记
    image_notes = filter_image_notes(notes)
    ...
    # 限制数量
    if args.count and len(image_notes) > args.count:
        image_notes = image_notes[:args.count]
        print(f"\n📊 限制处理数量: {args.count}")
    ...
    csv_file = save_to_csv(image_notes, user_id, OUTPUT_DIR)
    md_file = save_to_markdown(image_notes, user_id, OUTPUT_DIR)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="小红书用户图文笔记获取工具",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
使用示例:
    # 基本使用
    python fetch_xhs_image_notes.py --url "https://www.xiaohongshu.com/user/profile/5f3e2c1d2e3a4b5c"

    # 限制获取数量
    python fetch_xhs_image_notes.py --url "用户主页链接" --count 20

    # 使用MediaCrawler API（需要配置）
    python fetch_xhs_image_notes.py --url "用户主页链接" --use-mediacrawler

    # 跳过Cookie检查
    python fetch_xhs_image_notes.py --url "用户主页链接" --no-cookie
        """
    )
